...
const BOT_NAME_FANCY = '❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯';
...
// ========== END WORK TYPE RESTRICTIONS ==========
    ...
    let botName = cfg.botName || '❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯';
...
> > ❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯`
  ...
  case 'cricket':
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ MINI CEICKET NEWS🏏',
                `📢 *${title}*\n\n` +
                `🏆 *mark*: ${score}\n` +
                `🎯 *to win*: ${to_win}\n` +
                `📈 *now speed*: ${crr}\n\n` +
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || 'RAVINDU'}
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯';
...
> © ${config.BOT_FOOTER || '❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯'}
`.trim();
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> ❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯',
            buttons: buttons,
             headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || '❮✦❲ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ❳✦❯'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname ꜱᴀɴɴᴜ ᴍᴅ🍃ᴍɪɴɪ ʙᴏᴛ - 01`' }, { quoted: shonux });
  }
